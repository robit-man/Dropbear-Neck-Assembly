...
nats_fallback_state = {
    ...
    "subject": f"dropbear.camera.{_nats_subject_token}",
...
def _camera_fallback_payload(current_tunnel, process_running, listen_port):
            ...
            if found_url:
                ...
                try:
                    ...
                    if str(upnp_state.get("state") or "").strip().lower() == "active":
                        log(
                            f"[FALLBACK] UPnP camera endpoint ready: "
...
def tunnel_info():
    ...
    fallback_payload = _camera_fallback_payload(current_tunnel or "", process_running, listen_port)
...
def router_info():
    ...
    fallback_payload = _camera_fallback_payload(current_tunnel, process_running, listen_port)
    ...
    if lan_base:
        ...
        local_payload["lan_frame_packet_template"] = f"{lan_base}/frame_packet/<camera_id>"
...
def main():
    ...
    log(f"Starting camera router on {bind_url}")
